...
import { FilterProps, StatusParam } from "./types";

const AllCardsFilter = ({ alignContent, contentOffset }: FilterProps) => {
  ...
  return (
    <Popover open={isFilterOpen} onOpenChange={setIsFilterOpen}>
      ...
      <PopoverContent className="w-[26.25rem] rounded-[12px] px-5 pb-10 pt-5" align={alignContent} sideOffset={contentOffset}>
        <div>
          <div className="mb-6">
              ...
              <Collapsible open={isStatusOpen} onOpenChange={setIsStatusOpen}>
                <div className="flex items-center justify-between text-primary-gray">
                  ...
                  <CollapsibleTrigger asChild>
                    <Button
                      size="sm"
                      className={`bg-transparent p-0 text-primary-gray hover:bg-transparent ${isStatusOpen ? "rotate-180" : "rotate-0"} transition duration-200 ease-in-out`}
                    >
                ...
                <CollapsibleContent className="mt-2 grid gap-4 pl-4">
                  {statuses.map((status, i) => (
                    <div className="flex items-center gap-2" key={i}>
                      ...
                      <Label htmlFor={status.name.toLowerCase()} className="#252244">
                        {status.name}
                      </Label>
                    </div>
                  ))}
